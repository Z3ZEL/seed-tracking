...
if not(CONFIG["production"]):
    ...
    PARSER.add_argument("--calibrate", type=str, help="Enter in calibration mode, you need to give two input to --input arg separated by coma of the pictures of the main and the worker camera. Path are given like this ./output/*.jpg for instance", required=False, default=None, dest="calibrate")
    ...
    PARSER.add_argument("--calculate", action="store_true", help="Launch the seed velocity calculation depending on -i image separated by coma, main and worker. -o for output result", dest="calculate", default=False, required=False)
